'''
1. Реализовать класс «Дата», функция-конструктор которого должна принимать дату в виде строки
формата «день-месяц-год». В рамках класса реализовать два метода. Первый, с декоратором
@classmethod, должен извлекать число, месяц, год и преобразовывать их тип к типу «Число».
Второй, с декоратором @staticmethod, должен проводить валидацию числа, месяца и года
(например, месяц — от 1 до 12). Проверить работу полученной структуры на реальных данных.
'''

from datetime import datetime

class Data:
    def __init__(self, day_month_year):
        self.day_month_year = str(day_month_year)

    @classmethod
    def extract(cls, day_month_year):
        my_date = []

        for i in day_month_year.split():
            if i != '-': my_date.append(i)

        return int(my_date[0]), int(my_date[1]), int(my_date[2])

    @staticmethod
    def valid(day, month, year):

        if 1 <= day <= 31:
            if 1 <= month <= 12:
                if datetime.now().year >= year >= 0:
                    return f'***'
                else:
                    return f'Неправильный год: {year}'
            else:
                return f'Неправильный месяц: {month}'
        else:
            return f'Неправильный день {day}'

    def __str__(self):
        return f'{Data.extract(self.day_month_year)}'


date = Data('14 - 04 - 1964')
print(f'День космонавтики  {date}')
print(Data.valid(11, 11, 2052))
print(date.valid(11, 13, 2011))
print(Data.extract('23 - 03 - 2021'))
print(date.extract('11 - 11 - 2020'))
print(Data.valid(1, 11, 2000))

